...
main() {
  ...
  if [ -d "/etc/elytra" ]; then
    output "Elytra installation has been detected."
    warning "This will remove all the servers!"
    echo -e -n "* Do you want to remove Elytra (daemon)? (y/N): "
...
summary() {
  ...
  output "Uninstall Elytra? $RM_WINGS"
  print_brake 30
}
...
goodbye() {
  ...
  [ "$RM_WINGS" == true ] && output "Elytra uninstallation completed"
